...
function Home() {
  ...
  const [searchValue, setSearchValue] = useState('football'); // Default search term
  ...
  async function getNews() {
    ...
    try {
      const response = await fetch(
        `https://newsapi.org/v2/everything?q=${searchValue}&apiKey=${API_KEY}&pageSize=${article_number}`
      );
  ...
  return (
    <>
<h3 className="text-light fs-1 bg-dark m-0 p-4 text-center">Popular News</h3>
...
{newsData.map((newsItem, index) => (
    <a href={newsItem.url} target="_blank" rel="noreferrer" className="news-link">
      <div key={index} className="news-card  text-light out border-none mb-3">
        ...
        <div className="news_content p-1">
